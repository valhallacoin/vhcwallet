...
const (
	...
	// VHC/kB, this results in a maximum allowed high fee of 1 VHC/kB.
...
type Config struct {
	...
	FetchUtxoView func(*vhcutil.Tx, bool) (*blockchain.UtxoViewpoint, error)

	// BlockByHash defines the function use to fetch the block identified
	// by the given hash.
	BlockByHash func(*chainhash.Hash) (*vhcutil.Block, error)
	...
	// utxo view.
	CalcSequenceLock func(*vhcutil.Tx, *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error)
...
type Policy struct {
	...
	// MinRelayTxFee defines the minimum transaction fee in VHC/kB to be
...
type TxPool struct {
	...
	orphans       map[chainhash.Hash]*vhcutil.Tx
	orphansByPrev map[wire.OutPoint]map[chainhash.Hash]*vhcutil.Tx
	outpoints     map[wire.OutPoint]*vhcutil.Tx
...
func (mp *TxPool) insertVote(ssgen *vhcutil.Tx) error {
	msgTx := ssgen.MsgTx()
	...
	vote := vhcutil.IsFlagSet16(voteBits, vhcutil.BlockValid)
...
func (mp *TxPool) removeOrphan(tx *vhcutil.Tx, removeRedeemers bool) {
	txHash := tx.Hash()
...
//
// This function is safe for concurrent access.
func (mp *TxPool) RemoveOrphan(tx *vhcutil.Tx) {
	mp.mtx.Lock()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) addOrphan(tx *vhcutil.Tx) {
	...
	for _, txIn := range tx.MsgTx().TxIn {
		if _, exists := mp.orphansByPrev[txIn.PreviousOutPoint]; !exists {
			mp.orphansByPrev[txIn.PreviousOutPoint] =
				make(map[chainhash.Hash]*vhcutil.Tx)
		}
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) maybeAddOrphan(tx *vhcutil.Tx) error {
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) removeOrphanDoubleSpends(tx *vhcutil.Tx) {
	msgTx := tx.MsgTx()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) removeTransaction(tx *vhcutil.Tx, removeRedeemers bool) {
	txHash := tx.Hash()
	if removeRedeemers {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) RemoveDoubleSpends(tx *vhcutil.Tx) {
	// Protect concurrent access.
	mp.mtx.Lock()
	for _, txIn := range tx.MsgTx().TxIn {
...
func (mp *TxPool) addTransaction(utxoView *blockchain.UtxoViewpoint,
	tx *vhcutil.Tx, txType stake.TxType, height int64, fee int64) {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) checkPoolDoubleSpend(tx *vhcutil.Tx, txType stake.TxType) error {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) fetchInputUtxos(tx *vhcutil.Tx) (*blockchain.UtxoViewpoint, error) {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) FetchTransaction(txHash *chainhash.Hash) (*vhcutil.Tx, error) {
	// Protect concurrent access.
	mp.mtx.RLock()
...
// It should also set the vhcutil tree type for the tx as well.
func (mp *TxPool) maybeAcceptTransaction(tx *vhcutil.Tx, isNew, rateLimit, allowHighFees, rejectDupOrphans bool) ([]*chainhash.Hash, error) {
	msgTx := tx.MsgTx()
	txHash := tx.Hash()
...
//
// This function is safe for concurrent access.
func (mp *TxPool) MaybeAcceptTransaction(tx *vhcutil.Tx, isNew, rateLimit bool) ([]*chainhash.Hash, error) {
	// Protect concurrent access.
	mp.mtx.Lock()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) processOrphans(acceptedTx *vhcutil.Tx) []*vhcutil.Tx {
	var acceptedTxns []*vhcutil.Tx
	...
	for processList.Len() > 0 {
		...
		processItem := firstElement.(*vhcutil.Tx)
...
//
// This function is safe for concurrent access.
func (mp *TxPool) ProcessTransaction(tx *vhcutil.Tx, allowOrphan, rateLimit, allowHighFees bool) ([]*vhcutil.Tx, error) {
	// Protect concurrent access.
	mp.mtx.Lock()
	defer mp.mtx.Unlock()
	var err error
	...
	if len(missingParents) == 0 {
		...
		acceptedTxs := make([]*vhcutil.Tx, len(newTxs)+1)
...
//
// This function is safe for concurrent access.
func (mp *TxPool) RawMempoolVerbose(filterType *stake.TxType) map[string]*vhcjson.GetRawMempoolVerboseResult {
	mp.mtx.RLock()
	defer mp.mtx.RUnlock()

	result := make(map[string]*vhcjson.GetRawMempoolVerboseResult,
		len(mp.pool))
	bestHeight := mp.cfg.BestHeight()
	...
	for _, desc := range mp.pool {
		...
		mpd := &vhcjson.GetRawMempoolVerboseResult{
			Size:             int32(tx.MsgTx().SerializeSize()),
			Fee:              vhcutil.Amount(desc.Fee).ToCoin(),
...
func New(cfg *Config) *TxPool {
	return &TxPool{
		...
		orphans:       make(map[chainhash.Hash]*vhcutil.Tx),
		orphansByPrev: make(map[wire.OutPoint]map[chainhash.Hash]*vhcutil.Tx),
		outpoints:     make(map[wire.OutPoint]*vhcutil.Tx),
