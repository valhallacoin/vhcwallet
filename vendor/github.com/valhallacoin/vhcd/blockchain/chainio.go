...
import (
	...
	"github.com/valhallacoin/vhcd/blockchain/internal/dbnamespace"
...
func serializeSizeForMinimalOutputs(tx *vhcutil.Tx) int {
...
func putTxToMinimalOutputs(target []byte, tx *vhcutil.Tx) int {
...
// already there.
func dbMaybeStoreBlock(dbTx database.Tx, block *vhcutil.Block) error {
...
// The amount is never encoded into spent transaction outputs in Valhalla
...
func decodeSpentTxOut(serialized []byte, stxo *spentTxOut, amount int64, height uint32, index uint32) (int, error) {
	...
	// since in Valhalla we only need pkScript at most due to fraud proofs
...
// txouts.
func dbFetchSpendJournalEntry(dbTx database.Tx, block *vhcutil.Block) ([]spentTxOut, error) {
...
func (b *BlockChain) createChainState() error {
	...
	genesisBlock := vhcutil.NewBlock(b.chainParams.GenesisBlock)
...
// block for the provided node, deserialize it, and return a vhcutil.Block.
func dbFetchBlockByNode(dbTx database.Tx, node *blockNode) (*vhcutil.Block, error) {
	...
	block, err := vhcutil.NewBlockFromBytes(blockBytes)
	if err != nil {
		return nil, err
	}
