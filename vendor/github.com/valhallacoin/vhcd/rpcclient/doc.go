...
/*
Package rpcclient implements a websocket-enabled Valhalla JSON-RPC client.

Overview

This client provides a robust and easy to use client for interfacing
with a Valhalla RPC server that uses a mostly btcd/bitcoin core
style Valhalla JSON-RPC API.  This client has been tested with vhcd
(https://github.com/valhallacoin/vhcd) and vhcwallet
(https://github.com/valhallacoin/vhcwallet).

In addition to the compatible standard HTTP POST JSON-RPC API, vhcd and
vhcwallet provide a websocket interface that is more efficient than the standard
...
vhcd or vhcwallet by default.  However, configuration options are provided to
...
In contrast, the websocket-based JSON-RPC interface provided by vhcd and
vhcwallet only uses a single connection that remains open and allows
...
All notifications provided by vhcd require registration to opt-in.  For example,
...
example, the DebugLevel call is an extension only provided by vhcd (and
vhcwallet passthrough).  Therefore if you call one of these commands against
...
Also, it is important to realize that vhcd intentionally separates the wallet
functionality into a separate process named vhcwallet.  This means if you are
connected to the vhcd RPC server directly, only the RPCs which are related to
chain services will be available.  Depending on your application, you might only
need chain-related RPCs.  In contrast, vhcwallet provides pass through treatment
...
detected by type asserting the error in a *vhcjson.RPCError.  For example, to
detect if a command is unimplemented by the remote RPC server:

  amount, err := client.GetBalance("")
  if err != nil {
  	if jerr, ok := err.(*vhcjson.RPCError); ok {
  		switch jerr.Code {
  		case vhcjson.ErrRPCUnimplemented:
...
The following full-blown client examples are in the examples directory:
 ...
 - vhcdwebsockets
   ...
   block count
 - vhcwalletwebsockets
   Connects to a vhcwallet RPC server using TLS-secured websockets, registers
