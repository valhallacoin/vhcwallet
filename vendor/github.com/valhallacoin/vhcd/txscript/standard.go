...
func payToPubKeyHashEdwardsScript(pubKeyHash []byte) ([]byte, error) {
	edwardsData := []byte{byte(vhcec.STEd25519)}
	return NewScriptBuilder().AddOp(OP_DUP).AddOp(OP_HASH160).
...
func payToPubKeyHashSchnorrScript(pubKeyHash []byte) ([]byte, error) {
	schnorrData := []byte{byte(vhcec.STSchnorrSecp256k1)}
	return NewScriptBuilder().AddOp(OP_DUP).AddOp(OP_HASH160).
...
// valid SStxs.
func PayToSStx(addr vhcutil.Address) ([]byte, error) {
	...
	case *vhcutil.AddressPubKeyHash:
		...
		if addr.DSA(addr.Net()) != vhcec.STEcdsaSecp256k1 {
...
// valid SStxs.
func PayToSStxChange(addr vhcutil.Address) ([]byte, error) {
	// Only pay to pubkey hash and pay to script hash are
	// supported.
	scriptType := PubKeyHashTy
	switch addr := addr.(type) {
...
// valid SSGen txs.
func PayToSSGen(addr vhcutil.Address) ([]byte, error) {
	// Only pay to pubkey hash and pay to script hash are
	// supported.
	scriptType := PubKeyHashTy
	switch addr := addr.(type) {
...
// valid SSRtx.
func PayToSSRtx(addr vhcutil.Address) ([]byte, error) {
	// Only pay to pubkey hash and pay to script hash are
	// supported.
	scriptType := PubKeyHashTy
	switch addr := addr.(type) {
...
// an SStx.
func GenerateSStxAddrPush(addr vhcutil.Address, amount vhcutil.Amount, limits uint16) ([]byte, error) {
	// Only pay to pubkey hash and pay to script hash are
	// supported.
	scriptType := PubKeyHashTy
	switch addr := addr.(type) {
...
// specified address.
func PayToAddrScript(addr vhcutil.Address) ([]byte, error) {
	switch addr := addr.(type) {
	case *vhcutil.AddressPubKeyHash:
		if addr == nil {
			return nil, scriptError(ErrUnsupportedAddress,
...
func MultiSigScript(pubkeys []*vhcutil.AddressSecpPubKey, nrequired int) ([]byte, error) {
...
func ExtractPkScriptAddrs(version uint16, pkScript []byte,
	chainParams *chaincfg.Params) (ScriptClass, []vhcutil.Address, int, error) {
	if version != DefaultScriptVersion {
	...
	case PubKeyHashTy:
		...
		requiredSigs = 1
		addr, err := vhcutil.NewAddressPubKeyHash(pops[2].data,
			chainParams, vhcec.STEcdsaSecp256k1)
		if err == nil {
	...
	case PubKeyTy:
		...
		if err == nil {
			addr, err := vhcutil.NewAddressSecpPubKeyCompressed(pk, chainParams)
			if err == nil {
				addrs = append(addrs, addr)
			}
		}
	...
	case PubkeyAltTy:
		...
		switch suite {
		case vhcec.STEd25519:
			addr, err = vhcutil.NewAddressEdwardsPubKey(pops[0].data,
				chainParams)
		case vhcec.STSchnorrSecp256k1:
			addr, err = vhcutil.NewAddressSecSchnorrPubKey(pops[0].data,
				chainParams)
		}
		if err == nil {
			addrs = append(addrs, addr)
	...
	case StakeSubmissionTy:
		...
		var localAddrs []vhcutil.Address
	...
	case ScriptHashTy:
		...
		requiredSigs = 1
		addr, err := vhcutil.NewAddressScriptHashFromHash(pops[1].data,
			chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case MultiSigTy:
		...
		addrs = make([]vhcutil.Address, 0, numPubKeys)
...
func ExtractPkScriptAltSigType(pkScript []byte) (vhcec.SignatureType, error) {
	pops, err := parseScript(pkScript)
	if err != nil {
		return 0, err
	}
	...
	val := vhcec.SignatureType(valInt)
	switch val {
	case vhcec.STEd25519:
		return val, nil
	case vhcec.STSchnorrSecp256k1:
...
//
// NOTE: Atomic swaps are not considered standard script types by the vhcd
