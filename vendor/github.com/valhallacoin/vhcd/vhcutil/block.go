...
// Tx returns a wrapped transaction (vhcutil.Tx) for the transaction at the
...
// STx returns a wrapped transaction (vhcutil.Tx) for the stake transaction at
...
// Transactions returns a slice of wrapped transactions (vhcutil.Tx) for all
...
// instead provides easy access to wrapped versions (vhcutil.Tx) of them.
...
// STransactions returns a slice of wrapped stake transactions (vhcutil.Tx) for all
// stake transactions in the Block.  This is nearly equivalent to accessing the raw
// transactions (vhcwire.MsgTx) in the underlying wire.MsgBlock, however it
