...
func NewWIF(privKey chainec.PrivateKey, net *chaincfg.Params, ecType vhcec.SignatureType) (*WIF,
	error) {
	if net == nil {
...
func DecodeWIF(wif string) (*WIF, error) {
	...
	var ecType vhcec.SignatureType
	switch vhcec.SignatureType(decoded[2]) {
	...
	case vhcec.STEd25519:
		...
		ecType = vhcec.STEd25519
	case vhcec.STSchnorrSecp256k1:
		privKeyBytes := decoded[3 : 3+secp256k1.PrivKeyBytesLen]
		privKey, _ = secp256k1.PrivKeyFromScalar(privKeyBytes)
		ecType = vhcec.STSchnorrSecp256k1
	}
...
func (w *WIF) DSA() vhcec.SignatureType {
	return w.ecType
}
