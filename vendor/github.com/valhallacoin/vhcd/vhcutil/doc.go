...
A Block defines a Valhalla block that provides easier and more efficient
...
A Tx defines a Valhalla transaction that provides more efficient manipulation of
...
The Address interface provides an abstraction for a Valhalla address.  While the
most common type is a pay-to-pubkey-hash, Valhalla already supports others and
...
To decode/encode an address:
	...
	addr, err := vhcutil.DecodeAddress(addrString, defaultNet)
	if err != nil {
		fmt.Println(err)
		return
	}
