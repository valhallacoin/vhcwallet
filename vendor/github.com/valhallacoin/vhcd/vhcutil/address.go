...
func encodePKAddress(serializedPK []byte, netID [2]byte, algo vhcec.SignatureType) string {
	pubKeyBytes := []byte{0x00}

	switch algo {
	case vhcec.STEcdsaSecp256k1:
		pubKeyBytes[0] = byte(vhcec.STEcdsaSecp256k1)
	case vhcec.STEd25519:
		pubKeyBytes[0] = byte(vhcec.STEd25519)
	case vhcec.STSchnorrSecp256k1:
		pubKeyBytes[0] = byte(vhcec.STSchnorrSecp256k1)
	}

	// Pubkeys are encoded as [0] = type/ybit, [1:33] = serialized pubkey
	compressed := serializedPK
	if algo == vhcec.STEcdsaSecp256k1 || algo == vhcec.STSchnorrSecp256k1 {
...
type Address interface {
	...
	DSA(*chaincfg.Params) vhcec.SignatureType
...
func NewAddressPubKey(decoded []byte, net *chaincfg.Params) (Address, error) {
	if len(decoded) == 33 {
		...
		switch vhcec.SignatureType(suite) {
		case vhcec.STEcdsaSecp256k1:
...
func DecodeAddress(addr string) (Address, error) {
	...
	case net.PubKeyHashAddrID:
		return NewAddressPubKeyHash(decoded, net, vhcec.STEcdsaSecp256k1)

	case net.PKHEdwardsAddrID:
		return NewAddressPubKeyHash(decoded, net, vhcec.STEd25519)

	case net.PKHSchnorrAddrID:
		return NewAddressPubKeyHash(decoded, net, vhcec.STSchnorrSecp256k1)
...
// be 20 bytes.
func NewAddressPubKeyHash(pkHash []byte, net *chaincfg.Params, algo vhcec.SignatureType) (*AddressPubKeyHash, error) {
	var addrID [2]byte
	switch algo {
	case vhcec.STEcdsaSecp256k1:
...
// hash.
func (a *AddressPubKeyHash) DSA(net *chaincfg.Params) vhcec.SignatureType {
	switch a.netID {
	case net.PubKeyHashAddrID:
		return vhcec.STEcdsaSecp256k1
	case net.PKHEdwardsAddrID:
		return vhcec.STEd25519
	case net.PKHSchnorrAddrID:
		return vhcec.STSchnorrSecp256k1
	}
	return -1
}
...
func (a *AddressScriptHash) DSA(net *chaincfg.Params) vhcec.SignatureType {
	return -1
}
...
func NewAddressSecpPubKey(serializedPubKey []byte,
	...
	// from vhcec, but do it here to avoid API churn.  We already know the
...
// differs with the format.  At the time of this writing, most Valhalla addresses
...
// address.
func (a *AddressSecpPubKey) DSA(net *chaincfg.Params) vhcec.SignatureType {
...
// DSA returns the underlying digital signature algorithm for the
// address.
func (a *AddressEdwardsPubKey) DSA(net *chaincfg.Params) vhcec.SignatureType {
	return vhcec.STEd25519
}
...
// DSA returns the underlying digital signature algorithm for the
// address.
func (a *AddressSecSchnorrPubKey) DSA(net *chaincfg.Params) vhcec.SignatureType {
	return vhcec.STSchnorrSecp256k1
}
