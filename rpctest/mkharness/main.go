...
import (
	...
	"github.com/valhallacoin/vhcwallet/rpctest"
)
...
func main() {
	...
	fmt.Println("Command for node's vhcctl:")
	fmt.Printf("\tvhcctl -u %s -P %s -s %s -c %s\n", cn.User, cn.Pass,
		cn.Host, nodeCertFile)

	cw := primaryHarness.RPCWalletConfig()
	walletCertFile := primaryHarness.RPCWalletCertFile()
	fmt.Println("Command for wallet's vhcctl:")
	fmt.Printf("\tvhcctl -u %s -P %s -s %s -c %s --wallet\n", cw.User, cw.Pass,
