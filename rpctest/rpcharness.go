...
// Harness fully encapsulates an active vhcd process, along with an embedded
// vhcwallet to provide a unified platform for creating RPC-driven integration
// tests involving vhcd. The active vhcd node will typically be run in simnet
...
// spends. The active vhcd process is fully managed by Harness, which handles
...
func (h *Harness) SetUp(createTestChain bool, numMatureOutputs uint32) error {
	...
	// Start vhcwallet. This spawns a new process which will be managed
	...
	// Get a new address from the wallet to be set with vhcd's --miningaddr
	time.Sleep(5 * time.Second)
	var miningAddr vhcutil.Address
	for i := 0; i < 100; i++ {
...
// vhcd process belonging to this Harness instance. If the initial connection
